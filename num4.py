...
def save(filename: str, freqs: np.ndarray, rcs: np.ndarray):
    root = Element('data')
...
def plot(freqs: np.ndarray, rcs: np.ndarray):
...
def main():
    try:
        ...
        print(f"Параметры: D={D}м, fmin={fmin}Гц, fmax={fmax}Гц")

        freqs = np.logspace(np.log10(fmin), np.log10(fmax), n)
        rcs = RCS(D).calculate(freqs)

        save(f"rcs_{variant}.xml", freqs, rcs)
        plot(freqs, rcs)
